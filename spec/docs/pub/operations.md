...
| Embedding unavailable | Check `context-stilld doctor summary --json` fields `embedding.effectiveMode` and `embedding.daemon`; then check daemon URL, local Python/model paths, and `CONTEXT_STILL_EMBEDDING_DIMENSION` |
